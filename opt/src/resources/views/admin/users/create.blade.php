                    ...
                    <form action="{{ route('admin.users.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            <div class="flex items-center">
                                <input type="checkbox" id="is_admin" name="is_admin" value="1" {{ old('is_admin') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <label for="is_admin" class="ml-2 block text-sm font-medium text-gray-700">Admin User</label>
                            </div>
                            @error('is_admin')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
